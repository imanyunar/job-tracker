<template>
  <div
    v-if="isOpen"
    class="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4 bg-black/50 backdrop-blur-xs transition-opacity duration-300"
    @click="$emit('close')"
  >
    <div
      class="w-full max-w-md bg-[#F3F4F0] border border-[#C8D0CC] rounded-2xl shadow-2xl overflow-hidden transition-all text-[#1C2B2A]"
      @click.stop
    >
      <!-- Header -->
      <div class="p-6 text-center border-b border-[#C8D0CC] bg-[#ECEEEA] relative">
        <button
          @click="$emit('close')"
          class="absolute right-4 top-4 text-[#82918B] hover:text-[#1C2B2A] p-1 rounded-md transition-colors"
          aria-label="Tutup"
        >
        ...
        <div class="w-10 h-10 rounded-xl bg-[#1C2B2A] text-[#F3F4F0] flex items-center justify-center mx-auto mb-2.5 shadow-xs">
      ...
      <div class="p-6 space-y-4">
        ...
        <div
          v-if="errorMessage"
          class="bg-[#F8EFEF] border border-[#8B5A5A]/30 text-[#8B5A5A] px-3.5 py-2.5 rounded-md text-xs leading-relaxed"
        >
          {{ errorMessage }}
        </div>
        ...
        <div>
          <button
            ...
            class="w-full py-2.5 px-4 text-xs sm:text-sm font-semibold text-white bg-[#0A66C2] hover:bg-[#004182] active:bg-[#094c92] rounded-lg shadow-xs transition-colors flex items-center justify-center gap-2.5 cursor-pointer"
          >
        ...
        <form v-if="mode === 'login'" @submit.prevent="handleLogin" class="space-y-3.5 pt-1">
          ...
          <button
            type="submit"
            :disabled="submitting"
            class="w-full py-2.5 px-4 text-xs sm:text-sm font-semibold text-[#F3F4F0] bg-[#1C2B2A] hover:bg-[#2B3E3C] active:bg-[#14201F] rounded-lg shadow-sm transition-colors cursor-pointer disabled:opacity-50 mt-2"
          >
        ...
        <!-- Register Form -->
        <form v-else @submit.prevent="handleRegister" class="space-y-3 pt-1 max-h-[50vh] overflow-y-auto">
          <div>
